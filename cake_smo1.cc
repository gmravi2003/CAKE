...
double CAKESMO::CalculateFromScratch_(int row,int col){
...
void CAKESMO::FillUpBandwidthsOfBaseKernels_(){
  ...
  double min_bandwidth=plugin_bandwidth/mult_factor_for_bw_;
  double max_bandwidth=mult_factor_for_bw_*plugin_bandwidth;
...
bool CAKESMO::CheckForOptimality_(int position, double F_value){
  ...
  if(position_of_wsv2_in_I0_!=-1){
	...
	position_of_wsv1_in_I0_=CheckIfInI0_(current_row_,i_low_[current_row_]);
	if(position_of_wsv1_in_I0_==-1){
...
double CAKESMO::get_lscv_on_test_data(){
  ...
  quad_part-=reg_param_*la::LengthEuclidean(alpha_vec_);
...
void CAKESMO::PrintTestDensities(){
  ...
  FILE *fp;
  fp=fopen("qp_and_boosted_kde/mog2_qp_kde/cake_mog2.txt","w");
  for(int i=0;i<num_test_points_;i++){

    fprintf(fp,"%f\n",estimated_test_densities_[i]);
  }
  estimated_test_densities_.PrintDebug();
}

void CAKESMO::Init(Matrix &train_data,Matrix &test_data,double reg_param){
  ...
  //printf("num_train_points=%d..\n",num_train_points_);
  //printf("num_test_points=%d...\n",num_test_points_);
  //printf("True test densities length=%d..\n",true_test_densities_.length());
  //printf("num_base_kernels=%d..\n",num_base_kernels_);
  //printf("num of dims=%d..\n",num_dims_);
}
